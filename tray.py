...
self._onminimize = joinCommands(self.tk_deiconify,self._minimize)
...
def _minimize(self):
    ...
    if not getattr(self,'SysTrayIcon',None):
        self.SysTrayIcon = SysTrayIcon(
                                    ...
                                    menu_options,                   #右键菜单
                                    on_quit = self.onCloseWindow,   #退出调用
...
class SysTrayIcon (object):
    ...
    def activation(s):
        ...
        s.notify_id = None
        s.refresh(title = 'Hide！', msg = 'Click to open', time = 500)
    ...
    def destroy(s, hwnd = None, msg = None, wparam = None, lparam = None, exit = 1):
        ...
        if exit and s.on_quit: s.root.after(0,s.on_quit) #需要传递自身过去时用 s.on_quit(s)
...
DwmSetWindowAttribute.argtypes=[ctypes.c_void_p,ctypes.c_int,ctypes.c_void_p,ctypes.c_int]
DWMWA_CLOAK = 12
def hideWindow(hwnd):
    a = ctypes.c_int(1)
    p = ctypes.windll.user32.GetTaskmanWindow()
    while (q:=win32gui.GetParent(p))!=0:
        p=q
    #print(p)
    rgn = win32gui.CreateRoundRectRgn(0,0,1,1,0,0)
    win32gui.SetWindowLong(hwnd,win32con.GWL_HWNDPARENT,p)
    win32gui.SetWindowRgn(hwnd,rgn,0)
    DwmSetWindowAttribute(ctypes.c_void_p(hwnd),DWMWA_CLOAK,ctypes.cast(ctypes.byref(a),ctypes.c_void_p),4)
    return
def protectWindow(hwnd):
    ctypes.windll.user32.SetWindowDisplayAffinity(hwnd,17)
def showWindow(hwnd):
    a = ctypes.c_int(0)
    win32gui.SetWindowLong(hwnd,win32con.GWL_HWNDPARENT,0)
    win32gui.SetWindowRgn(hwnd,0,0)
    DwmSetWindowAttribute(ctypes.c_void_p(hwnd),DWMWA_CLOAK,ctypes.cast(ctypes.byref(a),ctypes.c_void_p),4)
def getHwnd(w):
    return win32gui.GetParent(w.winfo_id())
def bind_methods(w):
    hwnd=getHwnd(w)
    w.withdraw = lambda:hideWindow(hwnd)
    w.deiconify = lambda:showWindow(hwnd)
